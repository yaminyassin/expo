...
class AudioControlsService : MediaSessionService() {
  ...
  private fun setActivePlayableInternal(
    ...
    val previousPlayable = currentPlayable
    val previousListener = playbackListener
    appContext?.mainQueue?.launch {
      previousListener ?: return@launch
      previousPlayable?.player?.removeListener(previousListener)
    }

    playbackListener = null
    previousPlayable?.isActiveForLockScreen = false
    hideNotification()
